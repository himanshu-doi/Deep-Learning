...
#    theta3 = initialize_theta3(params = int((n_H2/2)*(n_W2/2)*l2))
    BN_input = np.dot(fc1,theta3) + bias3
    
    out, cache = batch_norm_forward(BN_input, gamma, beta, eps)
    ...
    dx, dgamma, dbeta = batch_norm_backward(d_out, cache)

    dtheta3 = np.dot(dx.T, fc1)
    dbias3 = np.mean(dx, axis = 0).reshape(1,10)    

    dfc1 = np.dot(theta3,dx.T)
    ...
    return dW1, dW2, db1, db2, dtheta3, dbias3, cost, probs, prob_label, acc, dgamma, dbeta 
...
def optimizer(batch,learning_rate,W1,W2,b1,b2,theta3,bias3,gamma,beta):
    ...
    [dW1, dW2, db1, db2, dtheta3, dbias3, cost_, probs_, prob_label, acc_,dgamma,dbeta] = grads
    ...
    return W1, W2, b1, b2, theta3, bias3, batch_cost, acc_, batch_accuracy, gamma, beta
...
def main_init(train_data,W1,W2,b1,b2,theta3,bias3,gamma,beta):
    cost = []
    accuracy = []
    for epoch in range(0, num_epochs):
        ...
        for batch in batches:
            ...
            output = optimizer(batch,learning_rate,W1,W2,b1,b2,theta3,bias3,gamma,beta)
            [W1, W2, b1, b2, theta3, bias3, batch_cost,acc_,batch_acc,gamma,beta] = output
...
W1_t,W2_t,b1_t,b2_t,theta3_t,bias3_t,cost_t,accuracy_t = main_init(t,W1,W2,b1,b2,theta3,bias3,gamma,beta)
...
def batch_norm_forward(data, gamma, beta, eps):
    m, features = data.shape
    mu = np.mean(data, axis = 0)     ## calculating mean along feature dimensions
    xmu = data - mu
    
    ## Calculating inverse of variance
    sq = xmu ** 2
    ...
    gammax = gamma * xhat      ## Implementing batch transformation using gamma and beta
    out = gammax + beta
...
def batch_norm_backward(d_out, cache):
    ...
    xhat,gamma,xmu,ivar,sqrtvar,var,eps = cache   ## Reading cache for BN back prop

    m, features = d_out.shape
    ...
    dsq = (1/m)*np.ones((m,features))*dvar
    dxmu2 = 2*xmu * dsq

    dx1 = (dxmu1+dxmu2)
    dmu = (-1)*np.sum(dxmu1+dxmu2, axis=0)

    dx2 = (1/m)*np.ones((m,features))*dmu
